...
public class MosaicProxy extends CaptureTransformer
    ...
    public MosaicProxy(CameraActivity activity) {
        ...
        if (params != null) {
            Display display = ((Activity)mGLSurfaceView.getContext()).getWindowManager().getDefaultDisplay();
            int screenWidth = screenWidth = display.getWidth();
            int screenHeight = screenHeight = display.getHeight();
            Point  previewSize = Util.getProperSize(params.getSupportedPreviewSizes(),screenHeight*1.0f/screenWidth);
            mPreviewWidth = previewSize.y;
            mPreviewHeight = previewSize.x;
        } else {
            mPreviewWidth = 480;
            mPreviewHeight = 640;
        }
