...
public class Util {
    ...
    public static Point getProperSize(List<Camera.Size> picSizeList, float v) {
        Collections.sort(picSizeList, new Comparator<Camera.Size>() {
            @Override
            public int compare(Camera.Size lhs, Camera.Size rhs) {
                int lPixels = lhs.height * lhs.width;
                int rPixels = rhs.height * rhs.width;
                if (lPixels > rPixels)
                    return 1;
    ...
    public static Point getFitSize(List<Camera.Size> sizes, int flag,float screenPor,int w1,int h1) {
        // 分辨率排序 大到小
        ...
        // 在Camera分辨率与屏幕分辨率宽高比不等的情况下 找出差距最小的分辨率
        // 由于Camera的分辨率是width>height 而portrait模式中 width < height
        // 因此在这里我们先交换Camera.Size的宽高
        ...
        while (it.hasNext()) {
            ...
            int maybeFlippedWidth = isCandidatePortrait ? height : width; // 取小
            int maybeFlippedHeight = isCandidatePortrait ? width : height; // 取大
            double sizePor = maybeFlippedWidth * 1.0 / maybeFlippedHeight;
            double porAbs = Math.abs(sizePor - screenPor);
            if (porAbs > 0.15) {
                it.remove();
                continue;
            }
            // 找到与屏幕相符的直接返回
            if (maybeFlippedWidth == w1 && height == h1)
                return new Point(maybeFlippedWidth, maybeFlippedHeight);
            // 找到比例相同的直接返回
            if (porAbs == 0) {
                return new Point(maybeFlippedWidth, maybeFlippedHeight);
            }
